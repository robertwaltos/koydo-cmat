...
import { emitPrismSignal, trackQuestionAttempt } from "@/lib/cmat/cortex-signals";

describe("Cortex Signals", () => {
  it("dispatches a custom event on window", () => {
    const spy = vi.fn();
    window.addEventListener("koydo:prism-signal", spy);
    emitPrismSignal({ event: "test", examId: "EXAM040" });
  ...
  it("tracks question attempts", () => {
    const spy = vi.fn();
    window.addEventListener("koydo:prism-signal", spy);
    trackQuestionAttempt("EXAM040", "q1", true, 5000);
    expect(spy).toHaveBeenCalledTimes(1);
